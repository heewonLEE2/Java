...
public class UDPServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			socket.setBroadcast(true); // 브로드캐스트 설정 true

			// 브로드캐스트 메세지
			String message = "헬로우 요우~ -히원-";

			// 전송을 위해 메시지 문자열을 byte[] 로 변환
			byte[] byteArr = message.getBytes();

			// 브로드캐스트 IP
			InetAddress broadcastIP = InetAddress.getByName("192.168.8.255");
			// 브로드캐스트 Port
			int boradcastPort = 9876;

			// 전송데이터를 담은 DatagramPacket
			DatagramPacket packet = new DatagramPacket(byteArr, // 전송할 바이트 배열
					byteArr.length, // 바이트 배열의 길이
					broadcastIP, // 브로드캐스트 IP
					boradcastPort); // 브로드캐스트 PORT
