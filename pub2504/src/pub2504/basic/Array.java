...
public class Array {
	...
	public static void main(String[] args) {
		...
		// 배열 선언 : 배열 변수명과 배열의 타입을 지정
		int[] intArr;
		// int intArr2[]; // 배열대괄호를 뒤에 쓰는 것도 가능하나 int 타입으로 오인될 수 있어 비추
		
		// 배열 초기화 : 배열요소의 개수만큼 메모리를 확보하고 각요소의 기본타입의 값으로 초기화
		// 자바에서는 배열을 항상 초기화한 후 사용할 수 있음
		intArr = new int[5]; // 크기(요소의 수 = length)는 5, 인덱스 범위는 0~4, 각 방에는 0(int가 초기화된)이 들어있음
		System.out.println(intArr[0]); // 0 => 0번방의 값
		...
		int intArrLeng =intArr.length; // for문 내에서 반복적으로 length할 필요 없음 연산 필요 없게
		...
		// 배열, 리스트내의 요소들을 반복하는 for 문
		// 인덱스는 필요없고 요소들만 반복해서 추출할때 사용
		for(int i : intArr3) {
