...
// CallableStatement 를 활용한 프로시져, 함수 호출
// CallableStatement 를 사용하면 보안, 성능, 모듈화의 큰 장점이 있으므로
// 적극적으로 활용하자!
...
public class JDBCTest4 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 프로시져 호출 방법 파라미터는 ? 사용해 표시
			// 문법 : {CALL 프로시져명(파라미터리스트에 해당하는 위치지정자들)}
			// 첫번째 IN 파라미터 : 자바에서 값 전달
			// 두번째 IN OUT 파라미터 : 자바에서 값 전달, 결과값을 DB에서 전달받음
			// OUT 파라미터인 경우 : registerOutParameter 를 통해 등록해 줘야 함
