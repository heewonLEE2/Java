...
public class SUBMAIN {
	...
	private static void selectMenu() throws SQLException, IOException {
		...
		case "2": {
			...
			TODO todo = new TODO(0, inputStr.substring(0, inputStr.indexOf(",")),
					inputStr.substring(inputStr.indexOf(",") + 1), null, "N");
			insertTodo(todo);
			selectTodoList();
		}
		case "3": {
	...
	private static void selectTodoList() throws SQLException, IOException {
		...
		String sql = " select tdno, tdwriter, tdcontent, tddate, tdyn from todo " + " order by tdno desc ";
		...
		while (rs != null && rs.next()) {
			...
			todoList.add(new TODO(rs.getInt("tdno"), rs.getString("tdwriter"), sb.toString(), rs.getTimestamp("tddate"),
					rs.getString("tdyn")));
		}
		if (br != null)
			br.close();
	...
	private static void insertTodo(TODO todo) throws SQLException {
		String sql = " insert into todo(tdno, tdwriter, tdcontent) " + " values(seq_todo.nextval, ?, ?) ";
		PreparedStatement pstmt = conn.prepareStatement(sql);
